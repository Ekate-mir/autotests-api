...
def assert_create_course_response(request: CreateCourseRequestSchema,
    ...
    assert_equal(request.preview_file_id, response.preview_file.id, "preview_file")
    assert_equal(request.created_by_user_id, response.created_by_user.id, "preview_file")
